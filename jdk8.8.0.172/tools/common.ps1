...
$uninstall_id = "180172"
$id = "a58eab1ec242421181065cdc37240b08"
$script_path = $(Split-Path -parent $MyInvocation.MyCommand.Definition)

 
# set $Forcei586 = $true to always do a 32 bit install without
# needing flags in the install command
# -jp
function use64bit($Forcei586 = $true) {
  if ($Forcei586) {
    return $false
  }
...
function download-jdk($Forcei586 = $true) {
...
function get-arch($Forcei586 = $true) {
...
function chocolatey-install($params, $Forcei586 = $true) {
  # I commented this out, since we'll just put the installer exe in locally
  # $jdk_file = download-jdk $Forcei586
  
  $jdk_file = Join-Path $script_path "jdk-8u172-windows-i586.exe"
  $arch = get-arch $Forcei586
  $java_home = get-java-home
  ...
  # removed the following, because we always want the same stuff
  # -jp
